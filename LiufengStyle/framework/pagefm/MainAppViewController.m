...
- (void)createTwo
{
    ...
    NSArray *arT = @[@"班组博客", @"新闻", @"公告", @"地图", @"聊天", @"测试6", @"测试7", @"测试8", @"测试9", @"测试10"];
...
- (void)addView2Page:(UIScrollView *)scrollV count:(NSUInteger)pageCount frame:(CGRect)frame
{
    for (int i = 0; i < pageCount; i++)
    {
        ...
        if(i==0){
            view=[self buildCPage:view];
        }else if (i==1){
            
          view=[self buildNewsPage:view];
        }else if (i==2){
            
           view= [self buildNoticePage:view];
        }else if (i==3){
            
           // [self buildNewsPage:scrollV tagNum:i];
        }else if (i==4){
            
             view= [self buildChatPage:view];
        }
...
-(void) buildETestPage:(UIScrollView *)scrollV  tagNum:(int)i
{
    //子页面初始化
    EGOTableViewPullRefreshDemoViewController *noticeView= [[EGOTableViewPullRefreshDemoViewController alloc] initWithNibName:@"EGOTableViewPullRefreshDemoViewController" bundle:nil];
    
    // NoticeViewController *noticeView = [[NoticeViewController alloc] init];
    CGRect newFrame=CGRectMake(scrollV.frame.size.width * i, 0, scrollV.frame.size.width, scrollV.frame.size.height);
    
    // newsView.catalog = 1;
    //    newsView.view.tag = i + 1;
    // noticeView.view.frame=newFrame;
    [self addChildViewController:noticeView];
    [self.view addSubview:noticeView.view];
    noticeView.view.frame = self.view.bounds;
    noticeView.view.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    
    
    
    
    [scrollV addSubview:noticeView.view];
    
}
-(UIView *) buildNewsPage:(UIView  *)view
{
    //子页面初始化
    NewsListView *newsListView= [[NewsListView alloc] initWithNibName:@"NewsListView" bundle:nil];
    //NSLog(@"====width: %f",scrollV.frame.size.width);
    // CGRect newFrame=CGRectMake(0, 0, scrollV.frame.size.width, scrollV.frame.size.height);
    newsListView.view.frame=view.frame;
    
    
    
    [self addChildViewController:newsListView];
    [self.view addSubview:newsListView.view];
    
    return newsListView.view;
    
}

-(UIView *) buildNoticePage:(UIView *) view
{
    //子页面初始化
    NoticeListView *noticeListView= [[NoticeListView alloc] initWithNibName:@"NoticeListView" bundle:nil];
      // CGRect newFrame=CGRectMake(0, 0, scrollV.frame.size.width, scrollV.frame.size.height);
    noticeListView.view.frame=view.frame;
    
    
    [self addChildViewController:noticeListView];
    [self.view addSubview:noticeListView.view];
    
    return noticeListView.view;
   // [scrollV addSubview:noticeListView.view];
    
}

-(UIView *) buildCPage:(UIView *) view
{
    //子页面初始化
    TestTableViewController *cview= [[TestTableViewController alloc] initWithNibName:nil bundle:nil];
    
   // CGRect newFrame=CGRectMake(0, 0, scrollV.frame.size.width, scrollV.frame.size.height);
    cview.view.frame=view.frame;
    

//    UINavigationController *navController = [[UINavigationController alloc]initWithRootViewController:cview];
    
    [self addChildViewController:cview];
    [self.view addSubview:cview.view];
    
    return cview.view;
   // [scrollV addSubview:cview.view];
    
}


-(UIView *) buildChatPage:(UIView *) view
{
    //子页面初始化
    //ChatViewController *chatview= [[ChatViewController alloc] initWithNibName:nil bundle:nil];
    
    MainTabViewController *mtabView = [[MainTabViewController alloc] init];
    
    mtabView.view.frame=view.frame;

    [self addChildViewController:mtabView];
    [self.view addSubview:mtabView.view];
    
    return mtabView.view;
